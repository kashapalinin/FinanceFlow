...
final public class FinanceService: IFinanceService {
    ...
    public func getCategories(for transactionType: TransactionType) throws -> [TransactionCategory] {
        ...
        return try getCategories(for: transactionType)
    }
    ...
    public func getCurrentBudget() throws -> Double {
        let budget = try getInitialBudget()
        let transactions = try getTransactions(for: DateInterval(start: .distantPast, end: .distantFuture))
